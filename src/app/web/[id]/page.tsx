...
const PageGeneration = ({ params }: { params: { id: string } }) => {
    return (
        <Suspense fallback={<PageLoader />}>
            <SessionProviderWrapper>
                <GeneratedPage params={params} />
            </SessionProviderWrapper>
        </Suspense>
    );
};
...
function GeneratedPage({ params }: { params: { id: string } }) {
    const searchParams = useSearchParams();
    const { data: session } = useSession();
    const pfUserId = params.id;
    ...
    const templateRef = useRef<HTMLDivElement>(null as any);
    ...
    const handleDownloadPDF = async () => {
        try {
            ...
            link.href = url;
            link.download = 'doctor_profile.pdf';
        ...
        <div className="relative">
            ...
            {session && (
                <div>
                    ...
                    >
                        {isGenerating ? 'Generating PDF...' : 'Download Profile PDF'}
                    </button> */}
                </div>
